...
- **额度是"每日"的**，按自然日重置，时区可配（服务器通常是 UTC，不配会提前 8 小时重置）。
- **默认额度全局配置、按用户可覆盖**：新用户不用逐个分配额度，`app_users.daily_token_quota`
...
# 新用户的默认每日 token 额度；0 = 不限
DEFAULT_DAILY_TOKEN_QUOTA = _int_env("DEFAULT_DAILY_TOKEN_QUOTA", 0)
...
QUOTA_SQL = [
    ...
    """,
    "alter table app_users add column if not exists daily_token_quota int",
...
def resolve_quota(override: int | None) -> int:
    """用户没单独配额度时用全局默认值。"""
    if override is None:
        return DEFAULT_DAILY_TOKEN_QUOTA
    return max(int(override), 0)


async def effective_quota(pool: AsyncConnectionPool, user_id: int) -> int:
    """这个用户今天生效的 token 额度（0 = 不限）。"""
    async with pool.connection() as conn:
        cur = await conn.execute(
            "select daily_token_quota from app_users where id = %s", (user_id,)
        )
        row = await cur.fetchone()
    override = row["daily_token_quota"] if row else None
    return resolve_quota(override)


async def today_usage(pool: AsyncConnectionPool, user_id: int) -> dict:
    """今天的用量；没有记录就是全 0。"""
    async with pool.connection() as conn:
        cur = await conn.execute(
            """
            select (now() at time zone %(tz)s)::date            as day,
                   coalesce(u.requests, 0)                      as requests,
                   coalesce(u.input_tokens, 0)                  as input_tokens,
                   coalesce(u.output_tokens, 0)                 as output_tokens
            from (select (now() at time zone %(tz)s)::date as d) t
            left join app_usage_daily u
                   on u.user_id = %(user_id)s and u.day = t.d
            """,
...
async def add_usage(
    ...
    """累加今天的用量（一次对话算一次 requests，token 从模型返回里取）。"""
    async with pool.connection() as conn:
        await conn.execute(
            """
